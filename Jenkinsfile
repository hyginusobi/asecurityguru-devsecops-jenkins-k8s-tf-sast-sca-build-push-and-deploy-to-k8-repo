  ...
  tools { 
   ...
   stages{
	...
	stage('Push') {
            steps {
                script{
                    docker.withRegistry('https://982081093653.dkr.ecr.us-west-2.amazonaws.com/', 'ecr:us-west-2:aws-credentials') {
                    app.push("latest")
                    }
                }
            }
	...
	stage('Kubernetes Deployment of OBI Bugg Web Application') {
	   steps {
